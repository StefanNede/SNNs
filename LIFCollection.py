...
class LIFCollection:
    ...
    def step(self, inputs):
        ...
        # NOTE: the code is weird here because in the dim=1 case inputs can be 2 things -> an array of floats where each value is specified (numpy shape: (N,)) or an array of just one value as same input to all neurones (numpy shape: ())
        #       but in the dim=D case inputs is NxD (numpy shape: (N,D))
        #       then the expression for I is just what works with numpy matrix operations (I lowkey just trial and errored to see what works and this does :) )
